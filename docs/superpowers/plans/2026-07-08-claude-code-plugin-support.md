...
export class TombstoneStore {
  ...
  private flush(): void {
    try {
      ...
      console.warn(`[akane] tombstone flush failed: ${String(err)}`);
    }
  }
}
...
describe("normalizeEvent", () => {
  ...
  test("SubagentStart/SubagentStop -> activity", () => {
    expect(normalizeEvent({ hook_event_name: "SubagentStart", session_id: "s1" }, now)?.kind).toBe("activity");
    expect(normalizeEvent({ hook_event_name: "SubagentStop", session_id: "s1" }, now)?.kind).toBe("activity");
  });
...
export function normalizeEvent(rawInput: unknown, now: number): AkaneClaudeEvent | null {
  ...
  switch (name) {
    ...
    case "SubagentStart":
    case "SubagentStop":
      // Subagents are treated as parent activity (SPEC §5.1); onToolSettled on an
      // untracked call would be equivalent to a re-arm anyway.
      return emit("activity");
    default:
      return null;
  }
...
export class ClaudeMonitor {
  ...
  tick(): void {
    ...
    try {
      ...
      this.deps.log("warn", `poll failed: ${String(err)}`);
    }
    for (const event of events) {
      try {
        ...
        this.deps.log("warn", `dispatch failed: ${String(err)}`);
      }
    }
...
フックの実際の stdin を記録するため、一時的に `AKANE_DEBUG=true` で hook の受信 JSON を stderr にダンプする（または `claude` の hook ドキュメントを参照）。`session_id` / `hook_event_name` / `tool_use_id` / `error_type` / Notification subtype の実フィールド名を確認。
...
4. 正常終了・入力待ち（PermissionRequest / Notification(permission_prompt)）で誤検知しないこと（SPEC §5.4-2）。
5. tmux 非起動環境でも monitor が落ちずログのみ残ること（AC #6）。
...
| AC #1 install/reload | Task 13 + Task 15（Step 1） |
| AC #2 userConfig 反映 | Task 3 + Task 15（Step 4） |
| AC #3–#5 stage1/stage2/max_pings | Task 10（integration）+ Task 15（Step 6） |
...
| AC #10 stdout 規律 | Task 6 / Task 10 |
...
**ギャップ**: なし。全 SPEC 節と 15 AC に少なくとも 1 タスクが対応。§7.5（マーケットプレイスカタログ repo）は本設計の非ゴール（SPEC §1.3）のため意図的にタスク化していない。
