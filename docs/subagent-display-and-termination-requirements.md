...
OpenCode ──session.created(parentID)──▶ akane event hook (src/index.ts)
                                          │
                                          ▼
                                  Subagent Registry (subagent-registry.ts)   ← single source of truth
                                          │  (server plugin プロセス内 in-memory Map: parent↔child マップ + ライフサイクル状態)
                        ┌─────────────────┼──────────────────────────┐
                        ▼                 ▼                          ▼
               Pane Manager           Session Terminator          (既存 watchdog 経路は無改変)
               (inline split +        (親活動再開 or grace で
               即時 attach / 上限4 /    session.delete)
               最古 evict)
...
- **単一レジストリ**: サブエージェントの追跡状態（親子マップ・生成時刻・pane 情報・完了保留フラグ）は新規 [`src/subagent-registry.ts`](../src/subagent-registry.ts) の **server plugin プロセス内 in-memory Map** に一元化する。追跡状態の消費者（Pane Manager / Session Terminator）はすべて server plugin と同一プロセスであり、TUI は別プロセスで本レジストリを参照しない。file-based アトミック共有ストア（[`src/shared-state.ts`](../src/shared-state.ts)）は watchdog/TUI 間の既存橋渡し専用で、本機能では無改変のまま利用しない。
- **モジュール分離**: 状態は共有しつつ、実装は独立モジュールに分ける（表示 / 削除 / 状態保持）。
...
`experimental.watchdog` 名前空間に以下を追加する（すべて optional・未指定は default、不正値は warn してデフォルト採用）。両機能とも**デフォルト無効（opt-in）**とする。設定の読み込み優先順位は **env > `experimental.watchdog` > project（flat 互換 alias）> defaults** とし、nested merge も同じ優先順位で統一する。

> `layout` / `eviction` / `attach` / `trigger` は設計書 D-4 により固定値（inline-split / oldest / eager+`--mini` / parent-resume-or-grace）として config 化しない。
...
新規 [`src/subagent-registry.ts`](../src/subagent-registry.ts) の **server plugin プロセス内 in-memory Map** に単一の Subagent レジストリを保持する。TUI は別プロセスであり本レジストリを参照しない。
...
- レジストリはプロセス内メモリで保持し、evict / 削除 / 停止時にエントリを整合的に更新・除去する。
