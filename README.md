...
アニメ『PSYCHO-PASS』の**監視官（常守朱）および特殊拳銃「ドミネーター」をコンセプト**とし、エージェントからのストリーム応答（色相）を常時モニタリングします。応答が一定時間途絶え、状態が濁った（ハングアップの疑いがある）と判定された場合、**Tmux連携による即時警告や対象エージェントへの自動介入（ノン・リーサル・パラライザー）、そしてゾンビプロセスの強制終了（デストロイ・デコンポーザー）を執行する「TUI上のドミネーター」としてシステムを正常な稼働状態へと導きます。**
...
- 🔫 **Auto-Intervention (ノン・リーサル・パラライザー)**: サイレンス（Stage 2）に達した場合、進行中のターンに割り込む `steer` デリバリー形式で自動Pingを即座に注入し、エージェントのハングアップを解消（非対応時は legacy queue 形式へグレースフルフォールバック）。
- 🧹 **Zombie Cleanup (デストロイ・デコンポーザー)**: 親プロセス（OpenCode）が終了したにもかかわらずバックグラウンドで走り続けているサブエージェントやPTYなどのゾンビプロセスを自動検知し、強制終了（Kill）してシステムリソースを解放。
